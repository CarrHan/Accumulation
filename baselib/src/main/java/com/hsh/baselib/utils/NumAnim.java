...
public class NumAnim {
    ...
    public static void startAnim(TextView textV, float num, long time) {
        if (num == 0) {
            textV.setText(FormatUtil.NumberFormat(num,2));
            return;
        }
    ...
    private static Float[] splitnum(float num, int count) {
        ...
        nums.add(0f);
        while (true) {
            float nextFloat = FormatUtil.NumberFormatFloat(
                    (random.nextFloat()*num*2f)/(float)count,
                    2);
            System.out.println("next:" + nextFloat);
            if (numtemp - nextFloat >= 0) {
                sum = FormatUtil.NumberFormatFloat(sum + nextFloat, 2);
                nums.add(sum);
                numtemp -= nextFloat;
            } else {
                nums.add(num);
    ...
    static class Counter implements Runnable {
        ...
        public void run() {
            ...
            view.setText(FormatUtil.NumberFormat(nums[i++],2));
            view.removeCallbacks(Counter.this);
