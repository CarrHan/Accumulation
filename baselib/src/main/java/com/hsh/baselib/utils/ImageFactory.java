...
//      return compress(bitmap, maxSize); // 这里再进行质量压缩的意义不大，反而耗资源，删除
     ...
     * @throws IOException
     */
    public Bitmap compressAndGenImage(String path,int maxSize) throws IOException {
        ...
        if(os.toByteArray().length/1024>10240){
            //如果图片大于10M
            os.reset();
            image.compress(Bitmap.CompressFormat.JPEG,30, os);
        }
        if(os.toByteArray().length/1024>5120){
            //如果图片大于10M
            os.reset();
